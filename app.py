	...
	if request.method == 'POST':
		...
		if data:
			...
			if sha256_crypt.verify(password_candidate, password):
				...
				session['prof'] = data[3]
				session['hash'] = sha256_crypt.hash(username)
...
def deleteTrainor():
	...
	if request.method == 'POST':
		...
		username = form.username.data
		cur.execute("SELECT username FROM trainors WHERE username != %s", [username])
		data= cur.fetchall()
		new = data[0][0]
...
def addMember():
	...
	for i in range(len(data2)):
		tup = (data2[i][0],data2[i][0])
		choices.append(tup)
	
	cur.execute("SELECT username FROM trainors")
	data3 = cur.fetchall()
	for i in range(len(data3)):
		tup = (data3[i][0],data3[i][0])
		choices2.append(tup)
...
def trainorDash():
	...
	if request.method == 'POST':
		...
		conn = get_db_connection()
		cur = conn.cursor()
		cur.execute("SELECT date FROM progress WHERE username = %s", [username])
		entered = []
		data2= cur.fetchall()
		for i in range(len(data2)):
			entered.append(data2[i][0])
...
def memberDash(username):
	...
	plan = (cur.fetchall())
	# %s", ['name']
	name = cur.execute("SELECT plan FROM members where username=%s", [username])
	names= cur.fetchall()
	print("..............///////",names)
	cur.execute("SELECT exercise, reps, sets FROM plans WHERE name =%s", names )
	scheme = cur.fetchall()
	print('------------schemmmma',scheme)
	cur.execute("SELECT date, daily_result, rate FROM progress WHERE username = %s ORDER BY date DESC", [username])
	data = cur.fetchall()
	print("--------------------",data)
	result = []
	for i in range(len(data)):
		result.append(int(data[i][2]))
	good = result.count(1)
	...
	cur.close()
	return render_template('memberDash.html',user = username, plan = plan, scheme = scheme, progress = data, good = good, poor = poor, average = average)
...
def edit_profile(username):
	...
	form.name.data = result[2]
	form.street.data = result[4]
	form.city.data = result[5]
	form.phone.data = result[6]
